  ...
  Widget build(BuildContext context) {
    return BlocConsumer<UpdateUserCubit, UpdateUserStates>(
      listener: (context, state) {
        ...
        } else if (state is UpdateUserStatesSuccess) {
          setState(() {
            isLoading = false;
          });
          showToast(
        ...
        } else if (state is UpdateUserStatesFailure) {
          setState(() {
            isLoading = false;
          });
          showToast(
        ...
        return Center(
          child: SingleChildScrollView(
            child: Padding(
              ...
              child: Column(
                children: [
                  if (widget.userModel.user?.profilePicture?.secureUrl != null)
                    CustomProfileImage(
                      onImageSelected: handleImageSelection,
                      image: widget.userModel.user!.profilePicture!.secureUrl!,
                    )
                    ...
                    CustomProfileImage(
                      onImageSelected: handleImageSelection,
                      image: 'default_image_url', // Provide a default image URL
                    ),
                  const SizedBox(height: 30),
                  CustomTextFormField(
                    keyboardType: TextInputType.name,
                    ...
                    labelText: 'Name'.tr(context),
                    colorText: Colors.black,
                    hintText: 'Enter your name', // Updated hint text
                  ),
                  const SizedBox(height: 10),
                  CustomTextFormField(
                    keyboardType: TextInputType.phone,
                    ...
                    labelText: 'Phone'.tr(context),
                    colorText: Colors.black,
                    hintText: 'Enter your phone number', // Updated hint text
                  ),
                  const SizedBox(height: 30),
                  CustomButon(
                    isLoading: isLoading,
                    ...
                    text: 'Update'.tr(context),
                    onPressed: () async {
                      print(widget.userModel.user?.email ?? 'No email');
                      if (selectedImage != null) {
                        cubit.fetchUserInfo(
                          profileImage: await MultipartFile.fromFile(
                            selectedImage!.path,
